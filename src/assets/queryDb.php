...
function getInventoryCompared($textId, $compared){
global $pg;
     $compared=explode(",",$compared);
    
    $prep = pg_prepare($pg,"getInventory","SELECT laeme.get_inventory_extended($1::integer, $2::integer[])");
    $ex = pg_execute($pg,"getInventory",array($textId,pg_arr($compared)));
    $rows = pg_fetch_all_columns($ex,0);
    return array("rows"=>$rows, "queryData"=>array());
...
global $pg;
    ...
    $morphids  = array_map("readMorphids", $slots);
...
function mapBoundSetStats($lits){
global $pg;
    $lits=explode(",",$lits);
    $prep = pg_prepare($pg,"getStatistics","SELECT laeme.get_map_bound_statistics($1::text[])");
    $ex = pg_execute($pg,"getStatistics",array(pg_arr($lits)));
    $rows = pg_fetch_all_columns($ex,0);
    return array("rows"=>$rows, "queryData"=>array()); 
}
...
function getSlotsCorresp($textIdA, $textIdB, $litA, $litB){
global $pg;

    $prep = pg_prepare($pg,"getSlots","SELECT laeme.get_slots($1::integer, $2::integer, $3::text, $4::text)");
    $ex = pg_execute($pg,"getSlots",array($textIdA, $textIdB, $litA, $litB));
    $rows = pg_fetch_all_columns($ex,0);
    return array("rows"=>$rows, "queryData"=>array()); 
}
...
function getSourceSplits($morphid,$langs){
global $pg;
    $langs= explode(",",$langs);
    $prep = pg_prepare($pg,"getSplits","SELECT laeme.get_splits($1::integer,$2::text[])");
    $ex = pg_execute($pg,"getSplits",array($morphid,pg_arr($langs)));
    $rows = pg_fetch_all_columns($ex,0);
    return array("rows"=>$rows, "queryData"=>array());
}

function linkForms($morphid){
global $pg;
    $prep = pg_prepare($pg,"linkForms","SELECT laeme.link_forms($1::integer)");
    $ex = pg_execute($pg,"linkForms",array($morphid));
    $rows = pg_fetch_all_columns($ex,0);
    return array("rows"=>$rows, "queryData"=>array());
}
...
function mapBoundSet($lits, $tags = ""){
global $pg;
    $lits=explode(",",$lits);

    $prep = pg_prepare($pg,"mapSet","SELECT laeme.map_set($1::text[])");
...
function getTitles(){
    global $pg;
     $prep = pg_prepare($pg,"getTitles","SELECT laeme.get_titles()");
    $ex = pg_execute($pg,"getTitles",array());
    $rows = pg_fetch_all_columns($ex,0);
...
function saveLitterae($split,$morphid,$pattern){
    ...
    return array("rows"=>['{"result":"saved"}'], "queryData"=>array());
}
